...
<script lang="ts">
import { defineComponent, ref, onMounted, computed, watchEffect, onBeforeUnmount } from "vue";
    ...
    setup(props) {
      ...
      const computeYearlyBillables = async () => {
        try {
          ...
          console.log(`[PieChart] ✅ Refetched and cached at ${new Date().toLocaleTimeString()}`);
        } catch (error) {
      ...
      onBeforeUnmount(() => {
        if (refetchInterval.value) {
          ...
          console.log("[PieChart] ⏹ Cleared refetch interval on unmount.");
        }
      });
